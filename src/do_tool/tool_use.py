...
class ToolUser:
    def __init__(self):
        self.llm_model_tool = LLM_request(
            model=global_config.llm_tool_use, temperature=0.5, max_tokens=1000, request_type="tool_use"
        )
    ...
    async def _build_tool_prompt(
        ...
        prompt += "你现在需要对群里的聊天内容进行回复，现在思考是否需要选择工具来对消息和你的回复进行处理，你是否需要额外的信息，比如回忆或者搜寻已有的知识，上网搜索你不知道的知识，或者了解你现在正在做什么。如果需要工具则进行选择，不需要则不选择"
        return prompt
