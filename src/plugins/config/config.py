...
class BotConfig:
    ...
    at_bot_inevitable_reply: bool = True  # @bot 必然回复
    ...
    tts_character: str = "芙宁娜"
    tts_batch_size: int = 15
    tts_api: str = "http://10.20.233.36:5000"
    tts_speed: float = 1.00
    ...
    def load_config(cls, config_path: str = None) -> "BotConfig":
        ...
        def message(parent: dict):
            ...
            config.language = msg_config.get("language", config.language)
        ...
        def tts(parent: dict):
            tts_config = parent["tts"]
            config.tts_enable = tts_config.get("enable", config.tts_enable)
            config.tts_api = tts_config.get("api_url", config.tts_api)
            config.tts_character = tts_config.get("character", config.tts_character)
            config.tts_batch_size = tts_config.get("batch_size", config.tts_batch_size)
            config.tts_speed = tts_config.get("speed", config.tts_speed)
        ...
        def groups(parent: dict):
            ...
            config.peek_groups = set(groups_config.get("peek_groups", []))
        ...
        include_configs = {
            ...
            "tts": {"func": tts, "support": ">=1.0.0", "necessary": False},
        }
