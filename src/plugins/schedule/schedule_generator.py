...
class ScheduleGenerator:
    ...
    def construct_daytime_prompt(self, target_date: datetime.datetime):
        ...
        prompt += "1. 学习安排 2. 日常活动和任务 3. 休息时间 4. 娱乐时间"
        prompt += "你一天都在做什么，从起床到睡眠，有什么发现和思考，"
        prompt +="注意，不要生成理想的日程安排，而是生成一个真实人的日程，可能存在熬夜、不按时吃饭等习惯。"
        prompt +="生成时，活动名只描述活动本身。内容用十几二十个字简单介绍活动内容。活动概括你一段时间内所关注的事情，不是生活中的细节。"
        prompt +="请按照时间顺序列出具体时间段和对应的活动，用时间段而不是时间点来表示时间，用JSON格式返回日程表，"
        prompt +="仅返回内容，不要返回注释，不要添加任何markdown或代码块样式，时间采用24小时制，格式为{时间段: 活动（地点，内容）,时间段: 活动（地点，内容）,...}"    # noqa: E501
        prompt += "直接返回日程，不要输出其他内容。"
        return prompt
    ...
    def construct_doing_prompt(self, time: datetime.datetime, mind_thinking: str = ""):
        ...
        prompt += f"现在是{now_time}，结合你的个人特点和行为习惯,用最多三句话安排你接下来一段时间要做什么，注意关注你今天的日程安排和想法，现实一点，不要浮夸"
        prompt += "生成一个真实人正在做的事情。"
        prompt +="生成时，概括你一段时间内所关注的事情，不是生活中的细节。"
        return prompt
...
async def main():
    ...
    scheduler.initialize(
        name="林晓麦",
        personality="你叫林晓麦，你19岁，是一个大二的女大学生，你有一头黑色短发，你会刷贴吧",
        behavior="你比较外向，一般熬夜比较晚，然后第二天早上10点起床吃早午饭",
        interval=60,
    )
