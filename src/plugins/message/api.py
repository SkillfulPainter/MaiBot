...
from src.plugins.config.config import BotConfig, update_config
    ...
    async def process_message(self, message: Dict[str, Any]):
        ...
        for handler in self.message_handlers:
            try:
                ...
                logger.info("检测到指令，消息传播被终止")
                return  # 直接返回，不执行后续处理器
                """
...
"""
async def custom_message_handler(message: Dict[str, Any]):
    # 判断消息内容是否符合特定条件
    if message.get("content") == "/test":
        # 执行自定义操作（例如记录日志、回复消息等）
        ...
        # 直接发送回复（示例）
        await global_api.send_message(reply_message)

        # 通过抛出特定异常阻断后续处理器（需适配原代码的异常处理）
        raise StopPropagation("拦截特定消息，停止传播")

# 注册到全局实例的处理器列表中（确保它是第一个处理器）
global_api.message_handlers.insert(0, custom_message_handler) """
