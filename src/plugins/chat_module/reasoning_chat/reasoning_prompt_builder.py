...
def init_prompt():
    Prompt(
        """注意，你是在扮演真人与网友在QQ上对话，回复内容要与真人说话习惯一致，思维不要太发散。
{relation_prompt_all}
{memory_prompt}
{prompt_info}
{schedule_prompt}
...
你正在{chat_target_2},现在请你读读之前的聊天记录，{mood_prompt}，分析是否在说你，你是否适合加入讨论，如果不在说你或者不适合加入，不返回任何内容。不要返回其他任何东西。
如果适合加入，请你给出回复内容。不对单个表情包或者图片进行回复。请注意把握聊天内容。
说{language_prompt}，尽量不要说你说过的话或者和聊天记录里说的话相似的话。请平淡地发言。发言要口语化日常化。在表达自己的想法时不需要先肯定别人，不要说类似于“什么什么怎么样，但是我认为..."的话，直接提出自己的想法。
你是在与网友聊天，所以请不要过多提及自身的背景，请不要过多提及自身在干什么，请不要过多提及自己之前的经历。
注意不要输出多余内容(包括前后缀，冒号和引号，括号，表情包，at或 @等)，只输出回复内容或不进行输出。输出内容不得超过三十个字。
{keywords_reaction_prompt}{prompt_ger}
{moderation_prompt}""",
    ...
    Prompt("\n你有以下这些**知识**：\n{prompt_info}\n请你**记住上面的知识**，之后可能会用到。\n", "knowledge_prompt")
    Prompt("你刚刚使用工具获取到了,这些信息：", "tool_prompt")
...
class PromptBuilder:
    ...
    async def _build_prompt(
        self, chat_stream, message_txt: str, tool_result_info: str, sender_name: str = "某人", stream_id: Optional[int] = None,
    ) -> tuple[str, str]:
        # 开始构建prompt
        ...
        # 请回复的平淡一些，简短一些，说中文，不要刻意突出自身学科背景，尽量不要说你说过的话，禁止输出简体中文之外的内容
        # {moderation_prompt}"""
        ...
        prompt = await global_prompt_manager.format_prompt(
            ...
            tool_prompt=tool_result_info,
            language_prompt=global_config.language,
        )
