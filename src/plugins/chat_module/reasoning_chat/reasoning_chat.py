...
from ...chat.utils import is_mentioned_bot_in_message, random_remove_punctuation
...
class ReasoningChat:
    ...
    async def process_message(self, message_data: str) -> None:
        ...
        if reply_probability != 1:
            ...
            if not chat.group_info:
                reply_probability = 1
                logger.info("当前为私聊，回复概率设为1")

            if reply_probability == 0:
                logger.info("麦麦的嘴被捂上了。。。")
        ...
        if random() < reply_probability:
            ...
            try:
                with Timer("生成回复", timing_results):
                    response_set = await self.gpt.generate_response(message, thinking_id, tool_result_info)
                    #response_set = random_remove_punctuation(response_set)
